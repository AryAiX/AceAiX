...
| 2 | **E-mail confirmation is not verifiable parental consent.** | It meets a "reasonable effort" standard in several regimes but would not satisfy COPPA — which is why new under-13 accounts are refused and any existing under-13 row is quarantined rather than given a child product. If the age floor ever moves, the consent mechanism has to move first. |
...
| 5 | **Native Reanimated/Worklets initialization is not covered by web CI.** | The real Expo-compatible packages and Metro resolution are checked in CI, but only an iOS and Android development/store build can prove the native JSI runtime and gestures initialize together. |
