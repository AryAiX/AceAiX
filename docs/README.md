...
| 25 | [`25-the-waitlist.md`](./25-the-waitlist.md) | The launch sign-up and the `/early-access` page: the two backends either form can post to, why an edge function sits between the page and the table, the guardian rule and the three places it is enforced, and double opt-in | Web + Backend + Marketing |

**01–09 describe the original web-first plan** and the React SPA in `web/`, which is now the
marketing and admin surface. **10–25 describe the product as it stands** after the September 2026
...
- **Touching meetups, the age gate or translation?** [21](./21-meetups-and-translation.md) —
  especially §2 before you loosen anything.
- **Adding a colour, a gradient or an animation?** [20](./20-colour-and-motion.md) — including
  the two things that render correctly on a phone and wrongly in the preview build, and
