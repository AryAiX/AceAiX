-- ============================================================
-- 0907/08 — Everybody gets a wallpaper
--
-- `athlete_profiles.cover_url` has existed since 0010 and nothing has ever
-- written to it: there was no way to set a cover, so every profile in the app
-- wore the same tinted band. It was also on the wrong table — a coach, a scout
-- and a club have profile pages too, and none of them has an athlete row.
--
-- So the column moves up to `user_profiles`, where it applies to everyone, and
-- `get_profile_bundle` answers with it. The athlete column stays and is read as
-- a fallback, because it is granted, public, and might already hold something
-- somebody set through the web app.
--
-- The rest of this file is `get_profile_bundle` re-stated with one field added.
-- Postgres has no way to add a key to a function's result without rewriting the
-- function, so the whole body is here, unchanged apart from that line.
-- ============================================================

alter table public.user_profiles
  add column if not exists cover_url varchar(1000);

/* Anything already set on the athlete row becomes the person's cover. */
update public.user_profiles up
   set cover_url = ap.cover_url
  from public.athlete_profiles ap
 where ap.user_id = up.id
   and up.cover_url is null
   and nullif(ap.cover_url, '') is not null;

/*
 * `user_profiles` has no table-wide UPDATE grant — 0904/02 withdrew it and
 * granted the owned columns back one by one, so a new column is unwritable
 * until it is named here. That is the design working, not an obstacle to it.
 */
...
begin
  ...
  return jsonb_build_object(
    'user', jsonb_build_object(
      ...
      'cover_url', coalesce(nullif(up.cover_url, ''), nullif(ap.cover_url, '')),
      'bio', up.bio,
      'city', up.city,
      'country', up.country,
